 ...
 * 类扫描器
 * <p>
 * 参考：{@link ClassScanner}@
 */
public class JwebClassScanner extends ClassScanner {
    ...
    public static final Set<Class> appClassesCache = new HashSet<>();
...
//    }
    ...
    public static List<Class> scanClass(boolean isInstantiable) {
        ...
        return appClassesCache.stream()
                .filter(JwebClassScanner::isInstantiable)
    ...
    private static void initAppClasses() {
        ...
        findClassPathsAndJarsByClassloader(jarPaths, classPaths, Thread.currentThread().getContextClassLoader());
        ...
        for (String classPath : classPaths) {
            ...
            if (JbootConfigManager.me().isDevMode()) {
                System.out.println("Jweb Scan ClassPath: " + classPath);
            }
        ...
        for (String jarPath : jarPaths) {
            ...
            if (JbootConfigManager.me().isDevMode()) {
                System.out.println("Jweb Scan Jar: " + jarPath);
            }
        ...
        try {
            ...
            if (urls != null) {
                for (URL url : urls) {
                    ...
                    String replace = path;
                    try {
                        replace = new File(path).getCanonicalPath().replace('\\', '/');
                    } catch (IOException e) {
                        System.out.println("WARN: CanonicalPath Failure: " + path + " -> " + e.getMessage());
                    }
                    if (!path.toLowerCase().endsWith(".jar")) {
                        classPaths.add(replace);
                    } else {
                        jarPaths.add(replace);
                    }
                }
            }
        } catch (Exception ex) {
    ...
    /**
     * 初始化ClassScanner的类缓存，避免ClassScanner自身加载类不全的问题
     */
    public static void initJbootClassScannerCache() {
        Field appClassesCache = null;
        try {
            appClassesCache = ClassScanner.class.getDeclaredField("appClassesCache");
            appClassesCache.setAccessible(true);
            JwebClassScanner.initIfNecessary();
            try {
                HashSet acc = (HashSet) appClassesCache.get(null);
                acc.addAll(JwebClassScanner.appClassesCache);
