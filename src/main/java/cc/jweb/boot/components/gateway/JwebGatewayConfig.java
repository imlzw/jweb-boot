 ...
 * @Date: 2020/3/22
 */
public class JwebGatewayConfig implements Serializable {
...
//    private String[] cookieContains;
    ...
    private JwebGatewayLoadBalanceStrategy gatewayLoadBalanceStrategy;
    ...
    public JwebGatewayInterceptor[] buildInterceptors() {
        ...
        if (inters == null) {
            synchronized (this) {
                if (inters == null) {
                    inters = new JwebGatewayInterceptor[interceptors.length];
                    for (int i = 0; i < interceptors.length; i++) {
                        JwebGatewayInterceptor interceptor = ClassUtil.newInstance(interceptors[i]);
                        if (interceptor == null) {
    ...
    public JwebGatewayLoadBalanceStrategy buildLoadBalanceStrategy() {
        ...
        if (gatewayLoadBalanceStrategy == null) {
            synchronized (this) {
                if (StrUtil.isBlank(loadBalanceStrategy)) {
                    this.gatewayLoadBalanceStrategy = JwebGatewayLoadBalanceStrategy.DEFAULT_STRATEGY;
                } else {
                    JwebGatewayLoadBalanceStrategy glbs = ClassUtil.newInstance(loadBalanceStrategy);
                    if (glbs == null) {
